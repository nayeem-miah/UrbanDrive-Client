...
const Cars: React.FC = () => {
    ...
    const {data:cardata= [],refetch,isLoading}=useQuery({
        queryKey:['car',currentPage,category,maxPrice,minPrice,sortOption],
        queryFn: async (page) =>{
            const response = await axiosPublic.get('/cars',{params:{page:currentPage,limit:6,category:category,minPrice:minPrice,maxPrice:maxPrice,sort:sortOption,seatCount: seatCount}});
...
>
    ...
    <option value="date-asc">Date Added: Oldest First</option>
                            ...
                            {cardata.map((car: Car) => (
                                <div key={car._id} className="card lg:card-side bg-base-100 shadow-xl rounded-2xl group">
                                    ...
                                    <div className="card-body w-full lg:w-2/3 flex-1">
                                        ...
                                        {car.rating > 0 ? (
                                            <p className='flex gap-1'>{car.rating}<MdOutlineStar className='text-[#f0bb0c] mt-1' /> ({car.trip_count} trips) <FaAward className='mt-1' /> All-Star-Host</p>
                                        ) : (
                                            <p>New listing</p>
                                        )}
            ...
            <div className="mx-auto text-center m-16">
                <button className="btn btn-active btn-primary mr-4 rounded-md"
